...
def create_module_graph(module_spec):
    ...
    with tf.compat.v1.Graph().as_default() as graph:
        resized_input_tensor = tf.compat.v1.placeholder(
            tf.compat.v1.float32, [None, height, width, 3])
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
    ...
    distort_result = tf.compat.v1.expand_dims(
        brightened_image, 0, name='DistortResult')
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    with tf.name_scope('input'):
        ...
        ground_truth_input = tf.compat.v1.placeholder(
            tf.compat.v1.int64, [batch_size], name='GroundTruthInput')
    ...
    with tf.name_scope('accuracy'):
        ...
        with tf.name_scope('accuracy'):
            evaluation_step = tf.compat.v1.reduce_mean(
                tf.compat.v1.cast(correct_prediction, tf.float32))
    ...
    if len(os.listdir(saved_model_dir)) == 0:
        ...
        with sess.graph.as_default() as graph:
            tf.compat.v1.saved_model.simple_save(
                ...
                outputs={'prediction': graph.get_tensor_by_name(
                    'final_result:0')},
                legacy_init_op=tf.compat.v1.group(
                    tf.compat.v1.tables_initializer(), name='legacy_init_op')
            )
    else:
        logging.error("{} directory should be empty, lease choose a different export directory, \
        or delete all the contents of the specified directory: {}".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--summaries_dir',
        type=str,
        default='tf/tf_files/training_summaries/my_model',
    ...
    parser.add_argument(
        '--saved_model_dir',
        type=str,
        default='tf/tf_files/saved_models/my_model',
    ...
    parser.add_argument(
        '--checkpoint_path',
        type=str,
        default='tf/tf_files/retrain_checkpoint',
